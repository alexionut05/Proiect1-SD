...
void Counting_Sort(int n, std::vector<long long>& v, int pow_base) {
    ...
    int count[20]; // count[i] = frecventa cifrei i la pasul curent (la primul apel ne intereseaza de
    // cate ori apare cifra i la unitati, apoi de cate ori apare la zeci...)
    for(int i = 0; i < 19; ++i) { // initializez
        count[i] = 0;
    }
   ...
   for(int i = 0; i < n; ++i) {
    int digit = v[i] / pow_base % 10; // preluam cifra care ne intereseaza
    count[digit + 9]++; // o normalizam si ii incrementam frecventa
   }
