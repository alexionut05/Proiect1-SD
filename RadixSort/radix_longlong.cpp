...
void Counting_Sort(int n, std::vector<long long>& v, int base, int pow_base) {
...
int Nr_Cifre(long long n, int base) {
    if(n == 0) return 1;
    int ct_cifre = 0;
    while(n) {
        n /= base;
...
void Radix_Sort(int n, std::vector<long long>& v, int base) {
...
void Citire(int &n, std::vector<long long> &v, int &base) {
    fin >> n;
    long long val;
    for(int i = 0; i < n; ++i) {
        fin >> val;
